...
APIKEY = os.environ.get('ANTHROPIC_API_KEY', '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')
...
SYSTEM_PROMPT = (
    ...
    "NO PICK when nothing qualifies. Always give gun-to-head picks. Never skip a batter.\n"
    "Use ONLY the Statcast numbers in the game data block -- never substitute training knowledge. "
    ...
    "S1 pitchers: name | gate | gap | one-line note. NO letter grades for pitchers.\n"
    ...
    "S5-S9: this is where all reasoning goes. Gun-to-head picks: 2 sentences max each.\n"
    "Parlays: list legs + estimated odds + one line reasoning. No tables.\n\n"
    + LOCKED_RULES
)
...
def compute_batter_score(b):
    ...
    return score, ' | '.join(pts), gap_flag, hr_cap
    ...
    for team in [away, home]:
        ...
        for b in batters:
            ...
            lines.append(
                ...
                f"SCORE={score}/4 | plat={platoon} | gap={gs}({gap_flag}){hr_cap} | "
      ...
      <div class="twrap">
        <table>
          <thead><tr>
            ...
            <th>xwOBA</th><th>wOBA</th><th>GAP</th><th>STATUS</th><th>SOURCE</th>
          </tr></thead>
...
function renderTable(data) {
  ...
  data.forEach(p => {
    ...
    tr.innerHTML =
      ...
      `<td class="na" style="font-size:.6rem">${src}</td>`;
